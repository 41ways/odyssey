...
export class Hud {
  ...
  update(player, { totalDamage, dt }) {
    ...
    this.setCharges(player.rollCharges >= 0 ? 3 : 3)
    for (let i = 0; i < this.pipEls.length; i++) {
      const filled = i < player.rollCharges
      const charging = i === player.rollCharges && player.rollCharges < 3
      this.pipEls[i].classList.toggle('empty', !filled)
      const f = this.pipEls[i].firstChild
      f.style.transform = `scaleX(${filled ? 1 : charging ? (player.rollTimer / 1.55) : 0})`
    }
