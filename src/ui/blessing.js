 ...
 */
const PORTRAIT = '/img/athena.webp'
...
/* 빛기둥 — 위에서 내려온다 */
#bless .beam { position:absolute; left:50%; top:-10%; width:46vw; height:120vh;
...
#bless .figure { position:absolute; left:50%; bottom:-3vh; transform:translate(-50%, -22%);
  height:94vh; opacity:0; transition:transform 1.2s cubic-bezier(.16,.9,.3,1), opacity 1.0s ease; }
...
#bless .figure .halo { position:absolute; left:50%; top:40%; width:80vh; height:80vh;
  transform:translate(-50%,-50%); z-index:-1; pointer-events:none;
  background:radial-gradient(circle, rgba(255,226,160,.40), rgba(180,140,60,.16) 38%, transparent 66%); }

#bless .said { position:absolute; left:50%; top:3.5vh; transform:translateX(-50%);
  width:min(720px, 92vw); text-align:center; opacity:0; transition:opacity .7s ease .5s; }
...
#bless .crown { display:flex; align-items:center; justify-content:center; gap:22px; margin-top:14px; }
#bless .laurel { width:26px; height:62px; background-image:${laurelURI('#e8c98a')};
  background-repeat:no-repeat; background-size:contain; }
#bless .laurel.r { transform:scaleX(-1); }
#bless .said h2 { font-family:var(--serif); font-size:52px; font-weight:700;
...
#bless .said .t { font-size:12.5px; letter-spacing:.34em; color:#c3a877; margin-bottom:16px; }
...
#bless .cards { position:absolute; left:50%; bottom:19vh; transform:translateX(-50%);
  display:flex; gap:16px; justify-content:center; opacity:0;
  transition:opacity .6s ease .85s; }
#bless.up .cards { opacity:1; }
#bless button { width:min(236px, 25vw); padding:0 0 20px; text-align:left; cursor:pointer;
...
#bless .bname { font-family:var(--serif); font-size:20px; font-weight:700;
...
#bless .foot { position:absolute; left:0; right:0; bottom:7.5vh; text-align:center;
  font-family:var(--serif); font-size:11px; letter-spacing:.34em; color:#7d6f52;
  opacity:0; transition:opacity .6s ease 1.1s; }
#bless.up .foot { opacity:1; }
`
