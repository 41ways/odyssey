 ...
 */
/**
 * 원래 2·4·6 이었다. `level.js` 의 곡선 주석은 "판마다 두 번, 총 스무 번"
 * 을 가정했는데(그러면 6 이 30%), 실제로 세 보니(13차 갱신 콘텐츠 감사)
 * 레벨업만으로는 한 회차 11번 안팎이다 — 은총·유물은 이 계열 집계에
 * 아예 안 들어간다(`stats.js` 의 `taken` 은 `UPGRADES` 전용). 6 을
 * 채우려면 11번 중 6번을 한 계열에 몰아야 하는데, 그러면 나머지 다섯도
 * 거의 다 그 계열이어야 해서 다른 계열은 일반조차 손을 못 댄다 —
 * "레전더리로 가는 길" 이 아니라 "그 판 내내 그것만 줍는 길" 이 됐다.
 * 2·3·5 로 내린다. 여전히 절반 넘게 한 계열에 걸어야 열리니 "몰아야
 * 본다" 는 정체성은 그대로고, 못 보는 카드는 아니게 된다.
 */
export const UNLOCK = [
  null,                              // 일반은 조건 없음
  { picks: 2 },                      // 레어
  { picks: 3, needsTier: 1 },        // 유니크
  { picks: 5, needsTier: 2 },        // 레전더리
]
